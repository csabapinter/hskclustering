...
fn merge_nodes_subset(
    ...
    for v in communities_of_size {
        ...
        let communities: Vec<(&IntSet<usize>, f64)> = filtered
            ...
            .collect();
        let weights = refinement_weights(&communities, theta);
...
/// Subtract before dividing so large positive gains cannot overflow exp() or
/// WeightedIndex's total. Scaling all weights equally preserves probabilities.
fn refinement_weights(communities: &[(&IntSet<usize>, f64)], theta: f64) -> Vec<f64> {
    let max_delta = communities
        .iter()
        .map(|(_, delta)| *delta)
        .fold(f64::NEG_INFINITY, f64::max);
    communities
        .iter()
        .map(|(_, delta)| ((delta - max_delta) / theta).exp())
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn refinement_sampling_does_not_overflow_for_large_gains() {
        let community = IntSet::default();
        let candidates = vec![(&community, 1000.0), (&community, 1001.0)];
        let weights = refinement_weights(&candidates, 0.3);
    ...
    #[test]
    fn refinement_sampling_preserves_relative_probabilities() {
        let community = IntSet::default();
        let candidates = vec![(&community, 0.1), (&community, 0.2), (&community, 0.3)];
        let weights = refinement_weights(&candidates, 0.3);
        let expected = ((0.1_f64 - 0.3) / 0.3).exp();
        assert!((weights[0] / weights[2] - expected).abs() < 1e-12);
        let shifted = vec![(&community, 1000.1), (&community, 1000.2), (&community, 1000.3)];
        let shifted_weights = refinement_weights(&shifted, 0.3);
        for (a, b) in weights.iter().zip(&shifted_weights) {
