...
impl<F: Float> GaussianMixtureModel<F> {
    ...
    /// Log probability density of each observation under the fitted mixture.
    pub fn score_samples<D: Data<Elem = F>>(&self, observations: &ArrayBase<D, Ix2>) -> Array1<F> {
        self.estimate_log_prob_resp(observations).0
    }
    ...
    fn estimate_log_prob_resp<D: Data<Elem = F>>(
        ...
        // Shift before exponentiating to avoid both overflow and underflow.
        let log_prob_norm = weighted_log_prob.map_axis(Axis(1), |row| {
            let max = row.iter().copied().fold(F::neg_infinity(), F::max);
            max + row.iter().map(|&x| (x - max).exp()).sum::<F>().ln()
        });
...
mod tests {
    ...
    #[test]
    fn log_normalization_handles_overflow_and_underflow() {
        let dimensions = 120;
        let covariance = Array2::<f64>::eye(dimensions) * 1e-8;
        let covariances = covariance.insert_axis(Axis(0));
        let precisions_chol = GaussianMixtureModel::compute_precisions_cholesky_full(&covariances).unwrap();
        let gmm = GaussianMixtureModel {
            covar_type: GmmCovarType::Full,
            weights: array![1.0],
            means: Array2::zeros((1, dimensions)),
            covariances,
            precisions: GaussianMixtureModel::compute_precisions_full(&precisions_chol),
            precisions_chol,
        };
        let observations = Array2::from_shape_fn((2, dimensions), |(i, _)| i as f64);
        let scores = gmm.score_samples(&observations);
        assert!(scores[0] > 700.0 && scores[1] < -700.0);
        assert!(scores.iter().all(|v| v.is_finite()));
        for p in gmm.predict_proba(&observations) { assert!((p - 1.0).abs() < 1e-12); }
    }
